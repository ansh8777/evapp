...
from .models import Car, Oil, EVCar, Subsidy, Apt, AptCharger, Charger, UsedCar, Danawa
...
def getCar(request):
    ...
    if search:
        # 브랜드 또는 모델명에서 검색어를 포함하는 차량 찾기
        cars = Car.objects.filter(
            Q(브랜드__icontains=search) | Q(모델명__icontains=search)
        ).values(
            '브랜드',
            '차종',
            '연료',
        ...
        # 검색 결과 반환
        return JsonResponse({'search_results': list(cars)})
...
def hybridPage(request):
    ...
    국산_브랜드 = ['현대', '기아', '제네시스', '쉐보레', 'KGM', '르노코리아', '대창모터스', '자일자동차', '제이스모빌리티', '디피코', '쎄보모빌리티', '마이브', 'EVKMC',
              'SMART EV', '이비온', '모빌리티네트웍스']
    브랜드_조건 = 국산_브랜드 if brand == 'k' else None
    ...
    params = [tuple(차종_조건_리스트), 최소_가격, 최대_가격]
    ...
    with connection.cursor() as cursor:
        cursor.execute(query_hybrid, params)
        rows = cursor.fetchall()
...
def searchPage(request):
    ...
    # Danawa 모델의 판매량 데이터를 주석으로 추가
    danawa_subquery = Danawa.objects.filter(모델명=OuterRef('모델명')).values('판매량', '번호')[:1]
    cars_qs = cars_qs.annotate(
        판매량=Coalesce(
            Subquery(danawa_subquery.values('판매량')),
            Value(0),
            output_field=IntegerField()
        ),
        번호=Coalesce(
            Subquery(danawa_subquery.values('번호')),
            Value(''),
    ...
    for car in cars:
        ...
        car_dict[model_name]['판매량'] = car['판매량']
        car_dict[model_name]['번호'] = car['번호']
    ...
    processed_cars.sort(key=lambda x: x['판매량'], reverse=True)
    ...
    paginator = Paginator(processed_cars, 20)  # 페이지당 20개 항목 표시로 변경
    ...
    return render(request, 'search.html', {
        'cars': cars_page,  # 'cars': processed_cars 에서 변경
        'query': query,
